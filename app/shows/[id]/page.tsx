...
// Hardened Import for Build Integrity
let seoData: any = {};
try {
  seoData = require("@/data/seo_master.json");
} catch (e) {
  console.warn("DCC Warning: Master Intelligence file missing.");
}
...
export default async function ShowPage(props: { params: Promise<{ id: string }> }) {
  ...
  // Hardened Venue Guard
  const isRedRocks = !show || show.venue.id === 196 || show.venue.name?.includes("Red Rocks") || !!localIntel;

  if (!isRedRocks) {
    return <div className="min-h-screen bg-black text-red-600 p-20 text-center font-black uppercase">Venue Mismatch</div>;
  }

  const performer = show?.performers[0] || { name: localIntel?.title || showId, image: localIntel?.image || "/hero-bg.jpg" };
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="relative h-[65vh] w-full overflow-hidden border-b border-white/10 shadow-2xl">
        <img 
          src={performer.image || localIntel?.image || "/hero-bg.jpg"} 
          alt={show?.title || performer.name}
          className="w-full h-full object-cover opacity-60 grayscale hover:grayscale-0 transition-all duration-1000"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent" />
        <div className="absolute bottom-0 left-0 right-0 p-12 flex flex-col md:flex-row justify-between items-end">
          <div className="max-w-5xl">
            ...
            <div className="flex flex-wrap gap-8 items-center bg-black/40 p-4 rounded-2xl backdrop-blur-md border border-white/5 w-fit">
              ...
              <a href="http://googleusercontent.com/maps.google.com/7" target="_blank" className="text-zinc-400 font-black uppercase tracking-widest text-[9px] hover:text-white transition underline decoration-blue-600 underline-offset-4 italic">Directions</a>
            </div>
          </div>
          <div className="text-right border-l border-white/20 pl-10 hidden lg:block bg-black/40 p-6 rounded-3xl backdrop-blur-md">
            <p className="text-zinc-500 uppercase font-black text-[9px] tracking-widest mb-1 italic">DCC Price Watch</p>
      ...
      <div className="grid grid-cols-12 gap-12 p-12">
        <div className="col-span-12 lg:col-span-4 space-y-12">
          ...
          <div className="p-10 rounded-[3rem] bg-zinc-900/50 border border-white/5 shadow-2xl">
            ...
            <p className="text-zinc-300 text-lg leading-relaxed font-medium italic">{performer.name} live performance profile is active. Secure shuttle booking recommended.</p>
          </div>

          {/* RESTORED SETLIST INTELLIGENCE */}
          <div className="p-10 rounded-[3rem] bg-zinc-900/50 border border-white/5 shadow-2xl">
        ...
        <div className="col-span-12 lg:col-span-8">
          <section id="booking" className="bg-zinc-900/40 p-2 rounded-[4.5rem] border border-white/5 min-h-[1200px] shadow-inner"><RezdyWidgets /></section>
        </div>
      </div>
    </main>
  );
