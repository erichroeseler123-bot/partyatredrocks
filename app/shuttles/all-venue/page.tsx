...
// 1. Full 21-Venue Database to match the /venues/[slug] routes
const venues = [
    ...
    <div className="min-h-screen bg-black text-white py-24 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-xl text-zinc-400 mb-16 font-medium max-w-3xl leading-relaxed">
          $50 per person ($250 min). Professional door-to-door transport for any group size, attending any major stage in Denver or Boulder. No passenger maximum.
        </p>

        {/* 2. Dynamic Venue Grid - All 21 Nodes */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
          {venues.map((v) => (
            <Link 
              key={v.slug} 
              href={`/venues/${v.slug}`} 
              className="group p-8 bg-zinc-900/40 border border-zinc-800 rounded-[2.5rem] hover:border-red-600 transition-all flex flex-col justify-between h-full"
            >
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-2xl font-black uppercase italic leading-tight">{v.name}</h3>
                  <span className="text-[10px] font-bold bg-zinc-800 px-2 py-1 rounded text-zinc-500 uppercase shrink-0">{v.loc}</span>
                </div>
                <p className="text-zinc-500 text-sm mb-6 font-medium">Professional transport and post-show waiting service for this venue.</p>
              </div>
              <span className="text-red-600 font-bold uppercase tracking-widest text-[10px] group-hover:underline">
                View Schedule & Intel →
              </span>
            </Link>
          ))}
        ...
        {/* 3. Corrected Call to Action: No Group Limits */}
        <div className="bg-red-600 p-12 rounded-[3rem] text-center shadow-2xl">
          ...
          <Link 
            href="/book-all-venue" 
            className="bg-white text-red-600 px-12 py-5 rounded-full font-black uppercase tracking-widest hover:bg-zinc-100 transition shadow-xl inline-block transform hover:scale-105"
          >
            Book City Service — $250 Min
          </Link>
          <p className="mt-8 text-[10px] text-red-900 font-bold uppercase tracking-[0.3em]">
            Guaranteed Pickup & Post-Show Waiting Standard
          </p>
        </div>
      </div>
    </div>
