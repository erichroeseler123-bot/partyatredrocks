...
export async function generateMetadata({
  ...
  return {
    title: `1stBank Center ${month.label} 2026 Concerts | Denver Show Schedule`,
    description: `1stBank Center ${month.label} 2026 schedule with artist links, show intel, and direct ride booking options.`,
    alternates: { canonical: `${SITE}/venues/1stbank-center/concerts/${month.slug}` },
  };
}

export default async function FirstBankMonthPage({
  params,
}: {
  params: Promise<{ month: string }>;
}) {
  ...
  const events = allEvents
    .filter((event) => event.venueId === "1stbank-center" && Number.parseInt(event.dateKey.split("-")[1] ?? "0", 10) === month.num)
    ...
    <main className="comic-page pt-24 pb-10">
      <section className="comic-wrap">
        <div className="comic-hero">
          <div className="comic-kicker">1stBank Center Month View</div>
          <h1 className="comic-title">1stBank Center {month.label} 2026 Concerts</h1>
          <p className="comic-copy">Monthly schedule for 1stBank Center in {month.label}. Drill into artist pages and book rides per show date.</p>
          <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3 sm:gap-4 mt-6 w-full px-4">
            <Link href="/venues/1stbank-center/concerts" className="comic-btn comic-btn-secondary w-full sm:w-auto min-w-[180px] text-center">
              Full 2026 Schedule
            </Link>
            <Link href="/find?venue=1stbank-center&qty=2" className="comic-btn comic-btn-primary w-full sm:w-auto min-w-[180px] text-center">
              Find a Ride
            </Link>
          </div>
        </div>
        ...
        <section className="comic-panel" style={{ marginTop: 16 }}>
          ...
          {events.length ? (
            ...
            <p className="comic-copy" style={{ marginTop: 8 }}>
              No 1stBank Center concerts were found for {month.label} 2026 in the current snapshot.
            </p>
          )}
        </section>
      </section>
