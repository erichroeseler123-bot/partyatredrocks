...
export default async function VenuePage({ params }: Props) {
  ...
  // ✅ Single source of truth: SeatGeek venue slug
  ...
  const eventsByMonth = events.reduce<Record<string, typeof events>>(
    (acc, event) => {
      const month = new Date(event.datetime).toLocaleString("en-US", {
        month: "long",
        year: "numeric",
      });
  ...
  return (
    <main className="min-h-screen bg-black text-white px-6 py-20">
      ...
      <header className="mb-16 border-b border-white/10 pb-10">
        ...
        <p className="mt-4 text-red-600 font-bold uppercase tracking-[0.4em] text-xs">
      ...
      <div className="space-y-24">
        {Object.entries(eventsByMonth).map(([month, monthEvents]) => (
          <section key={month}>
            <h2 className="text-red-600 font-black uppercase text-xs tracking-[0.4em] mb-6">
              {month}
            </h2>
