...
export async function generateMetadata({
  ...
  return {
    title: `Ball Arena ${month.label} 2026 Concerts | Denver Show Schedule`,
    description: `Ball Arena ${month.label} 2026 schedule with artist links, show intel, and direct ride booking options.`,
    alternates: { canonical: `${SITE}/venues/ball-arena/concerts/${month.slug}` },
  };
}

export default async function BallArenaMonthPage({
  params,
}: {
  params: Promise<{ month: string }>;
}) {
  ...
  const events = allEvents
    .filter((event) => event.venueId === "ball-arena" && Number.parseInt(event.dateKey.split("-")[1] ?? "0", 10) === month.num)
    ...
    <main className="comic-page pt-24 pb-10">
      <section className="comic-wrap">
        <div className="comic-hero">
          <div className="comic-kicker">Ball Arena Month View</div>
          <h1 className="comic-title">Ball Arena {month.label} 2026 Concerts</h1>
          <p className="comic-copy">Monthly schedule for Ball Arena in {month.label}. Drill into artist pages and book rides per show date.</p>
          <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3 sm:gap-4 mt-6 w-full px-4">
            <Link href="/venues/ball-arena/concerts" className="comic-btn comic-btn-secondary w-full sm:w-auto min-w-[180px] text-center">
              Full 2026 Schedule
            </Link>
            <Link href="/find?venue=ball-arena&qty=2" className="comic-btn comic-btn-primary w-full sm:w-auto min-w-[180px] text-center">
              Find a Ride
            </Link>
          </div>
        </div>
        ...
        <section className="comic-panel" style={{ marginTop: 16 }}>
          ...
          {events.length ? (
              ...
              {events.map((event) => (
                <article key={event.id} className="comic-panel">
                  ...
                  <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
                    ...
                    <Link href={`/find?date=${encodeURIComponent(event.dateKey)}&venue=ball-arena&qty=2`} className="comic-btn comic-btn-primary">
                      Ride Options
                    </Link>
                  </div>
                </article>
            ...
            <p className="comic-copy" style={{ marginTop: 8 }}>
              No Ball Arena concerts were found for {month.label} 2026 in the current snapshot.
            </p>
          )}
        </section>
      </section>
