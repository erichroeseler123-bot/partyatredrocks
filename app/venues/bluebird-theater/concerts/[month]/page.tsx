...
export async function generateMetadata({
  ...
  return {
    title: `Bluebird Theater ${month.label} 2026 Concerts | Denver Show Schedule`,
    description: `Bluebird Theater ${month.label} 2026 schedule with artist links, show intel, and direct ride booking options.`,
    alternates: { canonical: `${SITE}/venues/bluebird-theater/concerts/${month.slug}` },
  };
}

export default async function BluebirdMonthPage({
  params,
}: {
  params: Promise<{ month: string }>;
}) {
  ...
  const events = allEvents
    .filter((event) => event.venueId === "bluebird-theater" && Number.parseInt(event.dateKey.split("-")[1] ?? "0", 10) === month.num)
    ...
    <main className="comic-page pt-24 pb-10">
      <section className="comic-wrap">
        <div className="comic-hero">
          <div className="comic-kicker">Bluebird Theater Month View</div>
          <h1 className="comic-title">Bluebird Theater {month.label} 2026 Concerts</h1>
          <p className="comic-copy">Monthly schedule for Bluebird Theater in {month.label}. Drill into artist pages and book rides per show date.</p>
          <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3 sm:gap-4 mt-6 w-full px-4">
            <Link href="/venues/bluebird-theater/concerts" className="comic-btn comic-btn-secondary w-full sm:w-auto min-w-[180px] text-center">
              Full 2026 Schedule
            </Link>
        ...
        <section className="comic-panel" style={{ marginTop: 16 }}>
          ...
          {events.length ? (
            ...
            <p className="comic-copy" style={{ marginTop: 8 }}>
              No Bluebird Theater concerts were found for {month.label} 2026 in the current snapshot.
            </p>
          )}
        </section>
      </section>
