...
export default async function FiddlersGreenMonthPage({
  params,
}: {
  params: Promise<{ month: string }>;
}) {
  ...
  const events = allEvents
    .filter((event) => event.venueId === "fiddlers-green-amphitheatre" && Number.parseInt(event.dateKey.split("-")[1] ?? "0", 10) === month.num)
        ...
        <section className="comic-panel" style={{ marginTop: 16 }}>
          ...
          {events.length ? (
            ...
            <p className="comic-copy" style={{ marginTop: 8 }}>
              No Fiddler's Green Amphitheatre concerts were found for {month.label} 2026 in the current snapshot.
            </p>
          )}
        </section>
      </section>
