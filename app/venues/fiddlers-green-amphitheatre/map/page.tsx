...
export const metadata = {
  title: "Fiddler's Green Amphitheatre Map | Greenwood Village Venue Layout 2026",
  description:
    "Fiddler's Green Amphitheatre map showing stage, seating sections, entrances, parking, and transit for Greenwood Village shows.",
  alternates: { canonical: `${SITE}/venues/fiddlers-green-amphitheatre/map` },
};

export default async function FiddlersGreenMap() {
  const media = await getMediaIndex(2026);
  const mapImage = selectImageByPriority({
    spotifyImage: media?.venuesById?.["fiddlers-green-amphitheatre"]?.sources?.spotifyImage ?? null,
    ticketmasterImage: media?.venuesById?.["fiddlers-green-amphitheatre"]?.sources?.ticketmasterImage ?? null,
    seatgeekImage: media?.venuesById?.["fiddlers-green-amphitheatre"]?.sources?.seatgeekImage ?? null,
    localAsset: media?.venuesById?.["fiddlers-green-amphitheatre"]?.sources?.localAsset ?? null,
    fallback: "/images/venues/fiddlers-green-amphitheatre-map.jpg",
  });
    ...
    <main className="comic-page pt-24 pb-10">
      <section className="comic-wrap">
        <div className="comic-hero">
          ...
          <h1 className="comic-title">Fiddler's Green Amphitheatre Map</h1>
          <p className="comic-copy">
            Large outdoor amphitheatre layout with seating sections, entrances, parking, and transit points for 2026 shows.
          </p>
        ...
        <div className="mt-8 text-center">
          <img
            src={mapImage}
            alt="Fiddler's Green Amphitheatre Greenwood Village venue map showing stage, seating sections, entrances, parking, and transit"
            width={800}
            height={600}
            loading="lazy"
            decoding="async"
        ...
        <section className="comic-panel mt-8">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-3 mt-4 justify-center">
            <Link href="/venues/fiddlers-green-amphitheatre/best-time-to-arrive" className="comic-btn comic-btn-secondary">Best Time to Arrive</Link>
