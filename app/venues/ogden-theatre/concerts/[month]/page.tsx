...
export async function generateMetadata({
  ...
  return {
    title: `Ogden Theatre ${month.label} 2026 Concerts | Denver Show Schedule`,
    description: `Ogden Theatre ${month.label} 2026 schedule with artist links, show intel, and direct ride booking options.`,
    alternates: { canonical: `${SITE}/venues/ogden-theatre/concerts/${month.slug}` },
  };
}

export default async function OgdenMonthPage({
  params,
}: {
  params: Promise<{ month: string }>;
}) {
  ...
  const events = allEvents
    .filter((event) => event.venueId === "ogden-theatre" && Number.parseInt(event.dateKey.split("-")[1] ?? "0", 10) === month.num)
    ...
    <main className="comic-page pt-24 pb-10">
      <section className="comic-wrap">
        <div className="comic-hero">
          <div className="comic-kicker">Ogden Theatre Month View</div>
          <h1 className="comic-title">Ogden Theatre {month.label} 2026 Concerts</h1>
          <p className="comic-copy">Monthly schedule for Ogden Theatre in {month.label}. Drill into artist pages and book rides per show date.</p>
          <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3 sm:gap-4 mt-6 w-full px-4">
            <Link href="/venues/ogden-theatre/concerts" className="comic-btn comic-btn-secondary w-full sm:w-auto min-w-[180px] text-center">
              Full 2026 Schedule
            </Link>
        ...
        <section className="comic-panel" style={{ marginTop: 16 }}>
          ...
          {events.length ? (
              ...
              {events.map((event) => (
                <article key={event.id} className="comic-panel">
                  ...
                  <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
                    ...
                    <Link href={`/find?date=${encodeURIComponent(event.dateKey)}&venue=ogden-theatre&qty=2`} className="comic-btn comic-btn-primary">
                      Ride Options
                    </Link>
                  </div>
                </article>
            ...
            <p className="comic-copy" style={{ marginTop: 8 }}>
              No Ogden Theatre concerts were found for {month.label} 2026 in the current snapshot.
            </p>
          )}
        </section>
      </section>
