...
function CardUI({ c }: { c: Card }) {
  return (
    <div className="bg-surface border-soft shadow-soft rounded-3xl p-6 transition-all duration-300 hover:-translate-y-1 hover:glow-accent hover:shadow-2xl">
      ...
      <div className="mt-6">
        <Link href={c.href} className="inline-flex w-full items-center justify-center rounded-full border border-zinc-600/45 bg-surface/25 px-4 py-3 text-xs font-black uppercase tracking-[0.22em] text-strong hover:bg-surface/40 transition">
          {c.cta}
        </Link>
      </div>
    </div>
      ...
      <div className="bg-surface-strong border-soft shadow-soft rounded-[32px] p-7 md:p-10">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <div className="max-w-3xl">
            <p className="text-[12px] font-black uppercase tracking-[0.32em] text-muted">Transportation options</p>
            <h2 className="mt-3 text-3xl md:text-5xl font-black tracking-tight">Private Red Rocks transportation for your group</h2>
            <p className="mt-4 text-[16px] md:text-[18px] leading-relaxed text-soft">
              Published vehicle pricing, professional drivers, and timing built for show nights. Pick Private Suburban or upgrade to a private van.
            </p>
          </div>
        </div>
        ...
        <div className="mt-10">
          <div className="flex items-center justify-between gap-4">
            ...
            <Link href="/book" className="text-xs font-black uppercase tracking-[0.28em] text-soft hover:text-white">Get a quote →</Link>
          </div>

          <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {CARDS.map((c) => <CardUI key={c.title + c.price} c={c} />)}
          </div>
